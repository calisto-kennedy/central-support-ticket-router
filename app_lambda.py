...
def process_ticket(ticket_id):
    try:
        ...
        # Return the decision including the Current Handling Group
        return response
        print(f"Current Handling Group: {group_name}")
    
    except Exception as e:
        print(f"Error processing ticket {ticket_id}: {str(e)}")
        return f"Error processing ticket {ticket_id}: {str(e)}"

# Lambda Handler
def lambda_handler(event, context):
    try:
        # Extract ticket_id from the event
        ...
        if not ticket_id:
            return {
                'statusCode': 400,
                'body': json.dumps('ticket_id is required')
            }

        # Process the ticket using the ticket_id provided
